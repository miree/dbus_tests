  ...
  const Glib::RefPtr<Gio::DBus::MethodInvocation>& invocation)
{
  ...
  //////////////////////////////////////////////////////////////////////////////////////////////
  // Glibmm-version ( works find (no leaks) produces an annoying warning:
  //         "GLib-GObject-CRITICAL **: g_object_unref: assertion 'G_IS_OBJECT (object)' failed"
  //////////////////////////////////////////////////////////////////////////////////////////////
    // Glib::RefPtr<Gio::DBus::Message> message = invocation->get_message();
    // Glib::RefPtr<Gio::UnixFDList>    fd_list = message->get_unix_fd_list();
    // gint fd0, fd1;
    // fd0 = fd_list->get(0);
    // fd1 = fd_list->get(1);
    // std::cerr << "fd[0] = " << fd0 << "     fd[1] = " << fd1 << std::endl;
    // Glib::VariantContainerBase response;
    // // here would be the user function call ... 
    // invocation->return_value(response); // anser the call (unblocks the caller if it was a synchronous call)
    // std::cerr << "writing" << std::endl;
    //   std::vector<guint32> block(size,42);
    //   int nbytes = write(fd1, &size, sizeof(guint32));
    //   nbytes = write(fd1, &block[0],sizeof(guint32)*block.size()); // write to write end of pipe
    // std::cerr << "witing done " << nbytes << " bytes" << std::endl;
    // close(fd0);
    // std::cerr << "fd0 closed" << std::endl;
    // close(fd1);
    // std::cerr << "fd1 closed" << std::endl;

  //////////////////////////////////////////////////////////////////////////////////////////////
  // GLib-version ( works find (no leaks) but would be nicer to use the -mm wrapper of course)
  //  ... however, the version using the -mm wrapper produces the annoying warning:
  //         "GLib-GObject-CRITICAL **: g_object_unref: assertion 'G_IS_OBJECT (object)' failed"
  //////////////////////////////////////////////////////////////////////////////////////////////
    
    Glib::RefPtr<Gio::DBus::Message> message = invocation->get_message();
    GUnixFDList *fd_list  = g_dbus_message_get_unix_fd_list(message->gobj());
    gint fd0, fd1;
    if (fd_list)
    {
      std::cerr << "we got the fd_list" << std::endl;
      fd0 = g_unix_fd_list_get(fd_list,0,0);
      fd1 = g_unix_fd_list_get(fd_list,1,0);
      std::cerr << "fd[0] = " << fd0 << "   fd[1] = " << fd1 << std::endl;
    }
    else
    {
      std::cerr << "no fd_list :(" << std::endl;
    }
