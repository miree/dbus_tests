...
{
  //std::cerr << "server: on_method_call" << std::endl;
  ...
  else if (method_name == "GetArray")
  {
    ...
    int err = fd_list->append(fildes[1]);
    ...
    guint32 size;
    int nbytes = read(fildes[0], &size, sizeof(size));
    std::vector<guint32> block(size);
    nbytes = read(fildes[0], &block[0], sizeof(guint32)*block.size());
